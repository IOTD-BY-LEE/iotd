...
namespace eosio { namespace detail {
  ...
  struct vkt_txn_test_gen_status {
     string status;
  };
}}

FC_REFLECT(eosio::detail::vkt_txn_test_gen_empty, );
FC_REFLECT(eosio::detail::vkt_txn_test_gen_status, (status));
...
#define INVOKE_V_R_R_R(api_handle, call_name, in_param0, in_param1, in_param2) \
     ...
     eosio::detail::vkt_txn_test_gen_status result = { status };
   ...
   [this](string, string body, url_response_callback cb) mutable { \
      ...
      auto result_handler = [times_called{std::move(times_called)}, cb, body](const fc::exception_ptr& e) mutable {\
         ...
         if (e) {\
            ...
            cb(http_response_code, fc::variant(eosio::detail::vkt_txn_test_gen_empty())); \
         }\
      };\
      INVOKE \
   }\
...
struct vkt_txn_test_gen_plugin_impl {
   ...
   void create_test_accounts(const std::string& init_name, const std::string& init_priv_key, const std::function<void(const fc::exception_ptr&)>& next) {
      ...
      try {
         ...
         std::string user_priv_key = "5KkPDGBxyR92KJZBBzFmFtGqg34GMFzRQQU2HqamTRDDnpZP9hc";
         fc::crypto::private_key txn_test_receiver_A_priv_key = fc::crypto::private_key(user_priv_key);
         fc::crypto::private_key txn_test_receiver_B_priv_key = fc::crypto::private_key(user_priv_key);
         fc::crypto::private_key txn_test_receiver_C_priv_key = fc::crypto::private_key(user_priv_key);
   ...
   void send_transaction(std::function<void(const fc::exception_ptr&)> next, uint64_t nonce_prefix) {
      ...
      try {
         ...
         std::string user_priv_key = "5KkPDGBxyR92KJZBBzFmFtGqg34GMFzRQQU2HqamTRDDnpZP9hc";
         fc::crypto::private_key a_priv_key = fc::crypto::private_key(user_priv_key);
         fc::crypto::private_key b_priv_key = fc::crypto::private_key(user_priv_key);
...
vkt_txn_test_gen_plugin::vkt_txn_test_gen_plugin() {}
vkt_txn_test_gen_plugin::~vkt_txn_test_gen_plugin() {}
...
void vkt_txn_test_gen_plugin::plugin_startup() {
   app().get_plugin<http_plugin>().add_api({
      CALL_ASYNC(vkt_txn_test_gen, my, create_test_accounts, INVOKE_ASYNC_R_R(my, create_test_accounts, std::string, std::string), 200),
      CALL(vkt_txn_test_gen, my, stop_generation, INVOKE_V_V(my, stop_generation), 200),
      CALL(vkt_txn_test_gen, my, start_generation, INVOKE_V_R_R_R(my, start_generation, std::string, uint64_t, uint64_t), 200)
   });
}

void vkt_txn_test_gen_plugin::plugin_shutdown() {
   try {
      my->stop_generation();
   }
