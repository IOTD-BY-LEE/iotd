...
void accounting::deposit(account_name from, vector<account_record_content> content)
{
    ...
    asset temp_asset(0,S(4, IOTD));
    for (auto record_itr = acc_record.begin(); record_itr != acc_record.end(); ++record_itr)
    {
       eosio_assert(symbol_type(S(4, IOTD)).name() == record_itr->assets.symbol.name(), "Deposit asset symbol error");
    ...
    asset from_banlance  = token( N(eosio.token)).get_balance(from,symbol_type(S(4, IOTD)).name());
    ...
    for (auto record_itr = acc_record.begin(); record_itr != acc_record.end(); ++record_itr)
    {
        ...
        else
        {
            list.modify(itr, 0, [&](auto &tmp_record) {
                ...
                eosio_assert(symbol_type(S(4, IOTD)).name() == itr->assets.symbol.name(), "Deposit asset symbol error");
